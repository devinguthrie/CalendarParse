...
{
    /// <summary>
    /// <see cref="IOcrService"/> implementation that delegates to EasyOCR via a persistent
    /// Python subprocess. The subprocess keeps the model loaded across calls, so cold-start
    /// overhead (~5 s) is paid only once per benchmark run.
    ///
    ...
    /// </summary>
    public sealed class EasyOcrService : IOcrService, IDisposable
    {
        ...
        /// <param name="scriptPath">Absolute path to <c>easyocr_ocr.py</c>.</param>
        /// <param name="pythonExe">
        /// Python executable to use. Pass the full venv path when the system PATH
        /// does not include an environment with EasyOCR installed.
        /// </param>
        public EasyOcrService(string scriptPath, string pythonExe = "python")
        {
            ...
            _process = Process.Start(psi)
                ?? throw new InvalidOperationException("Failed to start EasyOCR Python subprocess.");
            ...
            if (ready != "READY")
                throw new InvalidOperationException(
                    $"EasyOCR subprocess did not signal readiness. First line was: {ready}");
        }

        public async Task<List<OcrElement>> RecognizeAsync(byte[] imageBytes, CancellationToken ct = default)
        {
            // EasyOCR requires a file path, not raw bytes.
            var tmpPath = Path.Combine(Path.GetTempPath(), $"easyocr_{Guid.NewGuid():N}.jpg");
            try
            {
